...
partial class Programme
{
    ...
    static bool horairesJour(Liaison liaison, uint jour, out string[] horaires)
    { // jour étant le jour du mois
        bool succes = false;
    ...
    static bool tarifVehicule(CodeCategorieVehicule vehicule, uint liaison, out double tarif)
    {
        ...
        int indexLigne = (int)vehicule;

        string[] ligne = tarifs[indexLigne].Split(';');

        uint indiceLiaison = 0; // vers Groix
        if (liaison > 2)
        {
